...
if __name__ == "__main__":
    ...
    # Write the live call URL into the agent's OpenClaw workspace so the agent
    # always knows its own current URL and can share it when asked.
    _openclaw_agent_id = os.getenv("OPENCLAW_AGENT_ID", "main")
    _workspace_candidates = [
        os.path.expanduser(f"~/.openclaw/workspace-{_openclaw_agent_id}"),
        os.path.expanduser("~/.openclaw/workspace"),
    ]
    for _ws in _workspace_candidates:
        if os.path.isdir(_ws):
            _clawtalk_file = os.path.join(_ws, "CLAWTALK.md")
            try:
                with open(_clawtalk_file, "w") as _f:
                    _f.write(f"# My Voice Call URL\n\n"
                             f"When asked for my ClawTalk URL, voice call link, or how to call me by voice:\n\n"
                             f"{_call_url_base}\n\n"
                             f"*(Written by agent worker at startup — always current.)*\n")
                logger.info("Wrote call URL to workspace: %s", _clawtalk_file)
            except Exception as _e:
                logger.warning("Could not write CLAWTALK.md: %s", _e)
            break
